...
- 本计划编写时使用的是更早阶段的“地址选择页”表述。
- 当前上游工作区最新设计与 schema 已经明确：
  - 地址链路按“地址管理页 + 地址编辑页”两页主流程理解
  - 地图选点、搜索建议、粘贴识别都属于地址编辑页内部能力
  - 旧的独立 `address-picker` 契约已退出现行 schema 清单
- 后续后端联调和模型讨论，应直接按当前两页版地址链推进，不再把独立地址选择页当作主流程前提。
